...
class RecommendationEngine:
    ...
    def load_data(self):
        ...
        print(f"Loaded {len(self.products_df)} products, {len(self.users_df)} users, {len(self.interactions_df)} interactions")
    ...
    def get_recommended_products(self, user_id, n_recommendations=5):
        ...
        most_common_products = [
            product_id for product_id, count in product_id_counts.most_common()
        ...
        if most_common_products:
            similar_user_recommendations = [
                product for product in self.products_df.to_dict('records')
        ...
        if remaining_slots > 0 and category_recommendations:
            ...
            for product in category_recommendations:
                ...
                if not any(r['_id'] == product['_id'] for r in combined_recommendations):
        ...
        if remaining_slots > 0:
            ...
            for product in popular_products:
                ...
                if (not any(r['_id'] == product['_id'] for r in combined_recommendations) and
                    product['_id'] not in interacted_product_ids):
                    combined_recommendations.append(product)
                    added += 1
...
def get_recommendations(user_id: str, n_recommendations: int = 5):
    ...
    try:
        ...
        recommendations = recommendation_engine.get_recommended_products(user_id, n_recommendations)
        
        if not recommendations:
            print(f"No recommendations found for user {user_id}. Returning popular items.")
            # Return popular items or other fallback recommendations
            popular_products = list(db.products.find().sort("rating", -1).limit(n_recommendations))
